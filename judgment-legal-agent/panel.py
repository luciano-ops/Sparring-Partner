"""Judgment Legal Agent Control Panel — web UI for controlling agent runs.

Usage:
    python3.11 panel.py              # Opens browser to http://localhost:5051
    python3.11 panel.py --port 8080  # Custom port

Provides sliders for Legal Domain, Client Sentiment, and Interaction Type
...
def _load():
    ...
    if _state["profiles"] is None:
        ...
        _state["profiles"] = [CaseProfile(**p) for p in data]
...
<title>Judgment Legal Agent</title>
<style>
  :root {
    --bg: #0f1117;
    --surface: #1a1d27;
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .slider-row {
    display: grid; grid-template-columns: 180px 1fr 50px 60px;
...
const GROUPS = {
  legal_domain: {
    title: "Legal Domain",
    labels: ["Contract & Commercial","Employment & Labor","IP & Technology","Regulatory & Compliance","Corporate & M&A","Litigation & Dispute"],
  },
  client_sentiment: {
    title: "Client Sentiment",
    labels: ["Anxious","Confused","Frustrated","Neutral","Reassured"],
  },
  interaction_type: {
    title: "Interaction Type",
    labels: ["Legal Consultation","Document Review","Risk Assessment","Research Request","Compliance Check"],
  },
};
  ...
  group.labels.forEach(label => {
    ...
    row.innerHTML = `
      ...
      <input type="range" min="0" max="100" value="${val}"
        oninput="onSlider('${groupKey}','${label.replace(/'/g, "\\\\'")}',this.value)">
      <div class="slider-pct" id="pct-${groupKey}-${CSS.escape(label)}">${val}%</div>
...
function onSlider(groupKey, changedLabel, newVal) {
  ...
  GROUPS[groupKey].labels.forEach(label => {
    const pctEl = document.getElementById(`pct-${groupKey}-${CSS.escape(label)}`);
    if (pctEl) pctEl.textContent = state[label] + '%';
    const slider = container = document.getElementById(`sliders-${groupKey}`);
    if (slider) {
      const inputs = slider.querySelectorAll('input[type=range]');
      const labels = GROUPS[groupKey].labels;
      const idx = labels.indexOf(label);
      if (idx >= 0 && inputs[idx]) inputs[idx].value = state[label];
    }
  });
...
def main():
    parser = argparse.ArgumentParser(description="Judgment Legal Agent Control Panel")
