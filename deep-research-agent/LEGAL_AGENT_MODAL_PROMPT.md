# Prompt: Deploy Legal Agent to Modal

Paste everything below this line into a new Claude chat:

---

I need you to deploy my **legal agent** app to Modal, following the exact same pattern as my deep-research-agent deployment that's already working. I'll give you all the reference code and context so you can do this without any additional info from me.
...
- **The legal agent app lives at**: `/Users/lucianoarroyo/test/legal-agent/` (create this directory if it doesn't exist)

## Available Modal Secrets (in `internal-agents-l` environment)

These are the secret names as they appear in the Modal dashboard:
- `Legal-Agent-Anthropic-Key` — **USE THIS ONE** (not the Research one)
- `Gemini-key`
- `JudgmentAPI_Key`
- `judgment-org-id`
...
- `Legal-Agent-Anthropic-Key` → exposes env var `Legal_Agent_Anthropic_Key` (NOT `ANTHROPIC_API_KEY`)
...
The tracing module provides lazy init so it's a no-op when credentials are missing. Project name for this legal agent should be `"Internal-Legal-Agent"`.

## What to Build

Create a `modal_legal.py` (the main Modal deployment file) plus the supporting app files. The legal agent should follow the EXACT same architecture as the deep-research-agent:

- **Multi-agent system**: Claude Sonnet (`claude-sonnet-4-20250514`) for the main legal agent, Gemini Flash (`gemini-2.0-flash`) for the requester simulator, Claude Haiku (`claude-haiku-4-5-20251001`) for simulated tools
...
### But adapted for LEGAL domain:

**Legal Research Domains** (replace the deep-research domains):
- Contract Law
...
**Legal Requester Personas** (replace the research personas):
- General Counsel
- Associate Attorney
- Paralegal
- Business Executive
...
**Legal Research Types** (replace the research types):
- Case Law Research
- Statutory Analysis
- Contract Review
- Due Diligence
...
**Legal Tools** (replace the research tools):
- `search_case_law` — search case law databases
- `read_statute` — read and extract from statutes/regulations
- `find_legal_precedents` — find relevant legal precedents
- `check_jurisdiction` — verify jurisdictional applicability
- `analyze_contract_clause` — analyze specific contract clauses
- `draft_legal_section` — draft a section of a legal memo/brief

**Domain Queries**: Create 15 queries per legal domain (90 total), similar depth/variety as the research agent queries.

**System Prompt for the Legal Agent**: Adapt to be a senior legal research analyst. Method: Search case law, analyze statutes, cross-reference precedents. Output: Cite all cases and statutes. Final reports: Executive Summary, Legal Analysis, Applicable Precedents, Risk Assessment, Recommendations, Sources.
...
1. Create the directory `/Users/lucianoarroyo/test/legal-agent/` if it doesn't exist
2. Create ALL the adapted files for the legal agent:
   - `modal_legal.py` — adapted Modal deployment (app name: `"legal-agent-eval"`, volume: `"legal-results"`, secrets use `Legal-Agent-Anthropic-Key` instead of `Research-Agent-Anthropic-Key`, env alias maps `Legal_Agent_Anthropic_Key` → `ANTHROPIC_API_KEY`, project name `"Internal-Legal-Agent"`, LAUNCH_DATE should be today's date)
   - `agent.py` — adapted for legal domain (system prompt, tool definitions, tool names)
   - `tools.py` — adapted legal tools (search_case_law, read_statute, find_legal_precedents, check_jurisdiction, analyze_contract_clause, draft_legal_section)
   - `models.py` — adapted enums and models for legal domain
   - `requester_simulator.py` — adapted persona styles for legal personas
   - `tracing.py` — COPY EXACTLY from reference (only change project name to `"Internal-Legal-Agent"`)
   - `run.py` — adapted for legal domain
   - `requirements.txt` — same as reference
3. Deploy to Modal: `MODAL_ENVIRONMENT=internal-agents-l /Users/lucianoarroyo/Library/Python/3.9/bin/modal deploy modal_legal.py`
4. Run a quick test: `MODAL_ENVIRONMENT=internal-agents-l /Users/lucianoarroyo/Library/Python/3.9/bin/modal run modal_legal.py --count 0` (debug tracing)
5. If tracing works, run a real test: `MODAL_ENVIRONMENT=internal-agents-l /Users/lucianoarroyo/Library/Python/3.9/bin/modal run modal_legal.py --count 1`
