"""CLI entrypoint for running health research sessions."""
...
from models import QueryProfile, HealthResearchTrace
...
def save_trace(trace: HealthResearchTrace, output_dir: str):
    """Save a health research trace to JSON."""
...
def run_session(profile: QueryProfile, verbose: bool = False) -> HealthResearchTrace:
    """Run a single health research session."""
    agent = HealthResearchAgent(profile=profile, verbose=verbose)
    return agent.run_session()


def main():
    parser = argparse.ArgumentParser(
        description="Health Research Agent - Run clinical research sessions"
    )
    parser.add_argument(
        "--count",
        type=int,
    ...
    traces: list[HealthResearchTrace] = []
    total_tokens = 0
    total_duration = 0.0

    if args.concurrency <= 1:
        for profile in tqdm(selected, desc="Health research sessions", disable=args.verbose):
            try:
        ...
        with ThreadPoolExecutor(max_workers=args.concurrency) as executor:
            ...
            with tqdm(total=len(selected), desc="Health research sessions") as pbar:
